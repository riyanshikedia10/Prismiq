...
Streamlit UI with Dashboard and Practice Mode.
Supports Data Scientist, Data Engineer, and Data Analyst roles.
"""
...
def _sidebar(driver):
    with st.sidebar:
        ...
        page = st.radio("Navigate", ["Dashboard", "Practice Mode"], label_visibility="collapsed")
        st.markdown("---")

        roles = get_roles(driver) if driver else ROLES
        role = st.selectbox("Role", roles)

        companies = (get_companies_for_role(driver, role) or get_companies(driver)) if driver else ROLES
    ...
    if overview["rounds"]:
        ...
        for i, r in enumerate(overview["rounds"]):
            with cols[i % len(cols)]:
                ...
                st.markdown(f'<div class="round-card" style="border-left-color:{color};"><strong>Round {r["round_order"]}</strong>: {r["name"]}<br><small>{r.get("duration","?")} min</small></div>', unsafe_allow_html=True)
    else:
        st.info(f"No round data for {role} at {company} yet. Run the extraction pipeline first.")

    st.markdown("### Required Skills")
    critical = [s for s in overview["skills"] if s["importance"] == "Critical"]
    high = [s for s in overview["skills"] if s["importance"] == "High"]
    medium = [s for s in overview["skills"] if s["importance"] == "Medium"]

    c1, c2, c3 = st.columns(3)
    for col, label, items, badge in [(c1, "Critical", critical, "badge-critical"), (c2, "High", high, "badge-high"), (c3, "Medium", medium, "badge-medium")]:
        with col:
            st.markdown(f"**{label}**")
            for s in items:
                st.markdown(f'<span class="skill-badge {badge}">{s["name"]}</span>', unsafe_allow_html=True)
...
def _practice(driver, company, role, experience):
    ...
    with col_g:
        if st.button("Generate Question", type="primary", use_container_width=True):
            with st.spinner("Generating..."):
                ...
                st.session_state.update(current_question=q, kg_context=ctx, actual_round=actual, feedback=None, reasoning=None)
    with col_h:
    ...
    st.markdown("---")
    st.markdown(f"**Round:** {q.get('round', actual)} | **Difficulty:** {q.get('difficulty')} | **Skills:** {', '.join(q.get('skills_tested', []))}")
    ...
    if eval_btn:
        ...
        with st.spinner("Evaluating..."):
            ...
            st.session_state.reasoning = explain_reasoning_path(company, ri["name"], ri.get("round_order", 1), q.get("skills_tested", []), role)
            st.session_state.practice_history.append({"question": q["question"], "score": fb["score"], "round": q.get("round", actual), "role": role})
