"""
Load processed JSON data into Neo4j Knowledge Graph.
Reads data/processed/{company}_data.json (Approach A) and
data/processed/leetcode_{company}.json + job_skills_{company}.json (Approach B)
...
def load_leetcode_questions(session, company_name: str, data: dict):
    ...
    q_count = 0
    for q in questions:
        q_id = q.get("id", f"{company_name}_LC{q_count+1}")
...
def load_job_skills(session, company_name: str, data: dict):
    """Load verified job-posting skills (Approach B, Source 2) into Neo4j."""
...
def load_kaggle_data(session, company_name: str):
    """Load Approach B data (LeetCode + Kaggle job skills) for a company."""
    lc_path = f"data/processed/leetcode_{company_name.lower()}.json"
    js_path = f"data/processed/job_skills_{company_name.lower()}.json"
    ...
    if lc_count or js_count:
        print(f"  [Approach B] {lc_count} LeetCode problems, {js_count} verified skills")
    else:
        print(f"  [Approach B] No Approach B data found for {company_name}")


def load_all(driver):
    """Load all company data from data/processed/ into Neo4j."""
    with driver.session() as session:
        ...
        for company in COMPANIES:
            ...
            if not os.path.exists(filepath):
                print(f"\n[{company}] SKIPPED — {filepath} not found")
                continue
